...
use crate::vamana::{Indexable, Distance, VectorAccessMethod};
...
impl KMeansAssignment
{
    fn new(k : usize, num_points : usize) -> Self
    {
        KMeansAssignment
        {
            ...
            local_indices : (0..num_points).map(|_| (AtomicUsize::new(0), AtomicUsize::new(0))).collect()
        }
    }
    ...
    {
        self.partition_counts_prefix_sum[partition_idx + 1] - self.partition_counts_prefix_sum[partition_idx]
    }
...
fn init_centroids<T : Indexable, D : Distance<T>, V : VectorAccessMethod<T>>(access_method : &V, k : usize) -> Array2<T>
{
    ...
    {
        c.as_slice_mut().unwrap().clone_from_slice(access_method.get_vec(row_id_for_cur_cent));
...
fn compute_closest_two_centroids<T : Indexable, D : Distance<T>>(p : &[T], centroids : &Array2<T>) -> (usize, usize)
{
    ...
    assert!(closest.0 != second_closest.0, "Point assigned to same partition twice");
...
) -> (usize, usize)
{
    ...
    for (ivec, vec) in access_method.iter().enumerate()
    {
        ...
        if a1 == centroid_id
        {
            centroid.iter_mut().zip(vec.iter()).for_each(|(x, y)| *x += y.clone());
            count_most_significant += 1;
        }
        if a1 == centroid_id || a2 == centroid_id
        {
            assert!(a1 != centroid_id || a2 != centroid_id, "Point {} assigned to centroid {} twice!", ivec, centroid_id);
            count_any += 1;
        }
    }
...
)
{
    assignments.assignments
        .par_iter_mut()
        .enumerate()
        .for_each(|(i, x)| *x = compute_closest_two_centroids::<T, D>(access_method.get_vec(i), centroids))
}
...
)
{
    centroids.outer_iter_mut()
        ...
        .for_each(|(i, ((mut x, count_any), count_most_significant))|
                   {
                       (*count_any, *count_most_significant) = update_single_centroid::<T, D, V>(&access_method, &assignments.assignments, i, &mut x);
                   });
}
...
pub fn kmeans<T : Indexable, D : Distance<T>, V : VectorAccessMethod<T>>(access_method : &V, k : usize) -> KMeansAssignment
{
    let num_points = access_method.num_points();
    ...
    loop
    {
        ...
        update_centroids::<T, D, V>(access_method, &mut assignments, &mut centroids, &mut histogram);
