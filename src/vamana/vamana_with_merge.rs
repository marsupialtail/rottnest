use rayon::prelude::*;
use ndarray::{Array2, s, concatenate, Axis};
...
pub trait Indexable : std::clone::Clone + num_traits::Zero + num_traits::FromPrimitive + std::ops::Div<Output = Self> + ndarray::ScalarOperand + std::ops::DivAssign + std::ops::AddAssign + std::marker::Sync + std::marker::Send {}
impl<T> Indexable for T where T : std::clone::Clone + num_traits::Zero + num_traits::FromPrimitive + std::ops::Div<Output = T> + ndarray::ScalarOperand + std::ops::DivAssign + std::ops::AddAssign + std::marker::Sync + std::marker::Send {}
...
unsafe impl<T : Indexable, D : Distance<T>, V : VectorAccessMethod<T>> std::marker::Send for VamanaIndex<T, D, V> {}
...
impl<T : Indexable, D : Distance<T>, V : VectorAccessMethod<T>> VamanaIndex<T, D, V>
{
    ...
    pub fn search(&self, ctx : &mut SearchContext, query : &[T])
    {
        ...
        while closest_unvisited_vertex < ctx.frontier.len()
        {
            ...
            ctx.frontier.sort_unstable_by(|x, y| x.1.partial_cmp(&y.1).unwrap());
            let new_frontier_size = dedup_frontier(&mut ctx.frontier).min(self.search_frontier_size());
    ...
    pub fn neighbors<'a>(&'a self, idx : usize) -> &'a [usize]
    {
        ...
        self.neighbors.slice(s![idx, 1..(num_neighbors + 1)]).reborrow().to_slice().unwrap()
    }

    fn neighbors_mut<'a>(&'a mut self, idx : usize) -> &'a mut [usize]
    {
        let num_neighbors = self.num_neighbors(idx);
        self.neighbors.slice_mut(s![idx, 1..(num_neighbors + 1)]).reborrow().into_slice().unwrap()
    }
...
impl BuildContext
{
    pub fn new<T : Indexable, D : Distance<T>, V : VectorAccessMethod<T>>(index : &VamanaIndex<T, D, V>) -> BuildContext
    {
        let distance_cache_size = std::cmp::max(index.num_points(), index.search_frontier_size() + index.max_num_neighbors());
        BuildContext
        {
    ...
    )
    {
        ...
        assert!(self.search_ctx.frontier.len() <= index.max_num_neighbors() + index.search_frontier_size());
        while closest_unvisited_vertex < self.search_ctx.frontier.len()
        {
            ...
            self.search_ctx.frontier.sort_unstable_by(|x, y| x.1.partial_cmp(&y.1).unwrap());
            let new_frontier_size = dedup_frontier(&mut self.search_ctx.frontier).min(index.search_frontier_size());
    ...
    )
    {
        ...
        self.cached_distances.sort_unstable_by(|x, y| x.1.partial_cmp(&y.1).unwrap());
...
fn randomize_edgelist<T : Indexable, D : Distance<T>, V : VectorAccessMethod<T>>(index : &mut VamanaIndex<T, D, V>)
{
...
fn compute_index_of_vector_closest_to_mean<T : Indexable, D : Distance<T>, V : VectorAccessMethod<T>>(access_method : &V) -> usize
{
    ...
    mean.iter_mut().for_each(|mi| *mi /= num_points_as_t.clone());

    let closest = access_method.iter()
        .enumerate()
        .map(|(i, x)|
             {
...
impl<'a, T : Indexable, V : VectorAccessMethod<T>> VectorAccessMethod<T> for PartitionedAccessMethod<'a, T, V>
{
    fn get_vec<'b>(&'b self, local_idx : usize) -> &'b [T]
    {
        let global_idx = self.partition_assignment.get_global_idx(self.partition_id, local_idx);
    ...
    fn num_points(&self) -> usize
    {
        self.partition_assignment.num_points_in_partition(self.partition_id)
    }

    fn iter<'b>(&'b self) -> impl Iterator<Item = &'b [T]>
    {
        self.partition_assignment.get_elements_in_partition(self.partition_id).iter().map(|i| self.underlying_access_method.get_vec(*i))
    }
...
pub fn build_index<T : Indexable, D : Distance<T>, V : VectorAccessMethod<T>>(access_method : V, params : IndexParams) -> VamanaIndex<T, D, V>
{
...
) -> VamanaIndex<T, D, V>
{
    ...
    let partition_indexes = (0..num_partitions).into_par_iter()
        .map(|partition_idx|
             {
        ...
        .for_each(|(iglobal, mut edgelist_including_length)|
                  {
                      ...
                      edgelist[0..a1_end].iter_mut().enumerate().for_each(
                          |(i, x)|
                          {
                              *x = partition_assignment.get_global_idx(a1, edges_a1[i]);
                          });
                      edgelist[a1_end..a2_end].iter_mut().enumerate().for_each(
                          |(i, x)|
                          {
