...
impl<'a> PList<'a> {
    pub fn new() -> Result<Self> {
        ...
        Ok(Self {
            ...
            elems: 0,
            last_flushed: 0
            // The actual compressed data Vec<u8> will be accessed differently
        })
    }

    pub fn search_compressed(compressed: Vec<u8>, indices: Vec<u64>) -> Result<Vec<Vec<u64>>> {
        // first read the last 8 bytes
        let compressed_plist_offsets_offset = u64::from_le_bytes(compressed[compressed.len() - 8 ..compressed.len()].try_into().expect("data corruption"));
        println!("{}", compressed_plist_offsets_offset);
        let mut decompressed_plists: Vec<u8> = Vec::new();
        let mut decompressor = Decoder::new(&compressed[..compressed_plist_offsets_offset as usize])?;
        decompressor.read_to_end(&mut decompressed_plists)?;

        println!("here");

        let mut decompressed_plist_offsets: Vec<u8> = Vec::new();
        let mut decompressor = Decoder::new(&compressed[compressed_plist_offsets_offset as usize .. compressed.len() as usize - 8])?;
        decompressor.read_to_end(&mut decompressed_plist_offsets)?;
        let mut decompressed_plist_offsets: Vec<u64> = bincode::deserialize(&decompressed_plist_offsets).unwrap();
        ...
        for index in indices {
            ...
            let plist: Vec<u64> = bincode::deserialize(&decompressed_plists[plist_offset as usize..plist_offset as usize + plist_size as usize]).unwrap();
            result.push(plist);
        }
    ...
    pub fn add_plist(&mut self, plist: &[u64]) -> Result<usize> {
        ...
        self.plist_offsets.push(serialized.len() + self.plist_offsets[self.plist_offsets.len() - 1]);
...
mod tests {
    ...
    fn test_big() -> Result<()> {
        ...
        let result = PList::search_compressed(result, vec![0]).unwrap();
        assert_eq!(result[0], sorted_numbers);
